...
function generateAbsolutePaths(options) {
  ...
  var absoluteBuildDirPath = path.join(buildDirPathFromProject, buildDirName, 'cache');
  ...
  var manifestPath = path.join(absoluteBuildDirPath, '..', 'build-manifest.json');
  return {
    absolute: {
  ...
  onPreBuild: function onPreBuild(_ref) {
    ...
    try {
      ...
      return Promise.resolve(utils.cache.restore(paths.absolute.buildDir)).then(function (success) {
        console.log(paths.absolute.buildDir + " " + (fs.existsSync(paths.absolute.buildDir) ? 'exists' : 'does not exist') + " on disk");
...
//# sourceMappingURL=netlify-plugin-cache-nextjs.cjs.development.js.map
